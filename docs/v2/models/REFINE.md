...
2. **Photographs.** `python3 scripts/fetch_reference_photos.py` — into
   `data/photos/<slug>/` (gitignored; reference only, never committed).
3. **Choose.** Read `ledger.json` beside this page and take the **three**
   buildings refined longest ago (never refined first). A building whose
   `sketchup` flag is set is refined in SketchUp (step 5); at most one of those
   per run.
4. **Refine each** (`MODELS.md` has the format):
...
5. **SketchUp** (flagged buildings): follow `scripts/sketchup/README.md` —
   ...
   really changed.
6. **If a gap is in the renderer**, not the model (a shape or material the
...
7. **Record.** In `ledger.json`, set each refined building's `refined` to
   today's date and add one to its `passes`; add a line to its `log`.
8. **Check and publish.** Open the site locally (`python3 -m http.server
