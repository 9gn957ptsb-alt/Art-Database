...
| `scripts/build_theatre.py` | Writes `docs/v2/theatre/` — **the theatre at the Folger**: twelve scenes from Shakespeare (the Ghost, the witches, the balcony, the Tempest's storm, Yorick, the quarrel, the Dream's wood, the sleepwalking, the Forum, Lear's storm, Falstaff, Ophelia) and the library itself, drawn as isometric pixel art on clods of **DIRT** after the reference sheet the artist sent (not blocks: the voxel figures read as Lego and were dropped, 23 Sep 2026). `scripts/theatre/` is a small signed-distance-field renderer at the picture's own resolution: hand-picked ramps, light from the upper left, narrow-band Bayer dither, selective outlines, faces put in pixel by pixel, the earth read cell by cell off `dirt-land.png` (the sea's off `dirt-sea.png`). Each scene is 4 loop frames plus 4 per speaker, packed as the first frame and the rectangles that change; `theatre.json` holds the lines (Shakespeare's), who says them and where their heads are. In the Folger view (`land.js`, "the theatre") a scene rises out of the soil a row at a time, its people say their lines, it sinks back and the next is dealt; pressing it takes the next line, the bill steps back or on. Scenes are edited in `scripts/theatre/scenes.py` and rebuilt with `python3 scripts/build_theatre.py [--only key]` (about six minutes on four cores). |
