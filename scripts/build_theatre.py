...
def main():
    ...
    for k in keys:
        spec = spec_by(k)
        states = [None] + sorted({line[0] for line in spec["lines"]})
        for who in states:
            for t in range(frames):
                jobs.append((k, t, who, geo[k][0], geo[k][1]))
    with Pool(args.jobs) as pool:
        done = pool.map(frame_of, jobs, chunksize=1)
    got = {(k, t, who): img for k, t, who, img in done}
    ...
    for k in keys:
        ...
        speakers = sorted({line[0] for line in spec["lines"]})
        for who in speakers:
            for t in range(frames):
